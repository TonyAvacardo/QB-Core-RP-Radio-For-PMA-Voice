...
-- Function to update player's radio frequencies
function UpdateRadioFrequencies()
    local PlayerData = QBCore.Functions.GetPlayerData()

    -- Debugging: --print PlayerData
   -- --print("Updating frequencies for player:", PlayerData.citizenid)
   -- --print("Job:", PlayerData.job.name)
   -- --print("Gang:", PlayerData.gang.name)

    -- Remove all frequencies first
    exports["rp-radio"]:RemovePlayerAccessToFrequencies(1,2,3,4,5,6,7,8, 999, 998, 997, 996)

    -- Job-based frequencies
    if PlayerData.job.name == 'police' then
        exports["rp-radio"]:GivePlayerAccessToFrequencies(1,2,3,4,6,7,8)
        --print("Access given to police frequencies")
    elseif PlayerData.job.name == 'ambulance' then
        exports["rp-radio"]:GivePlayerAccessToFrequencies(1,2,3,4,5,6,7,8)
        --print("Access given to ambulance frequencies")
    end

    -- Gang-based frequencies
    if PlayerData.gang.name == 'Gang 1' then
        exports["rp-radio"]:GivePlayerAccessToFrequencies(999)
        --print("Access given to Gang 1 frequency")
    elseif PlayerData.gang.name == 'Gang 2' then
        exports["rp-radio"]:GivePlayerAccessToFrequencies(998)
        --print("Access given to Gang 2 frequency")
    elseif PlayerData.gang.name == 'Gang 3' then
        exports["rp-radio"]:GivePlayerAccessToFrequencies(997)
        --print("Access given to Gang 3 frequency")
    elseif PlayerData.gang.name == 'Gang 4' then
        exports["rp-radio"]:GivePlayerAccessToFrequencies(996)
        --print("Access given to Gang 4 frequency")
    end
end
...
Citizen.CreateThread(function()
    ...
    UpdateRadioFrequencies()

    -- Continuously update frequencies
    while true do
        Citizen.Wait(1000) -- Increased wait time to reduce unnecessary updates
        UpdateRadioFrequencies()
    end
end)

-- Event listener for player loading
...
-- Event listener for gang changes
